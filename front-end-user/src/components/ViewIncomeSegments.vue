...
<div>
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb mt-4 mx-4">
          ...
          <li class="breadcrumb-item active" aria-current="page">Income Segments</li>
        </ol>
    </nav>
      ...
      <div class="card-body">
        <div class="row">
          <div class="col-md-3">
              <h3 class="m-4">nama</h3>
                <ul class="list-group ml-3">
                  ...
                  <li class="list-group-item active side-tab"><a>Income Segments</a></li>
          ...
          <div class="col-md-9 mt-5">
              <h2 class="m-4">Income Segments</h2>
                <table class="table table-borderless">
                    <tbody>
                        <tr>
                            <th scope="row">Nama Perusahaan</th>
                            <td>{{incSeg.companyName}}</td>
                        </tr>
                        <tr>
                            <th scope="row">Bidang Industri</th>
                            <td>{{incSeg.industry}}</td>
                        </tr><tr>
                            <th scope="row">Posisi</th>
                            <td>{{incSeg.position}}</td>
                        </tr>
                        <tr>
                            <th scope="row">Pendapatan Perbulan</th>
                            <td>{{incSeg.monthlyIncome}}</td>
                        </tr>
                        <tr>
                            <th scope="row">Alamat Kerja</th>
                            <td>{{incSeg.workAddrDetails}}</td>
                        </tr>
                        <tr>
                          <th scope="row">Area Alamat</th>
                          <td>
                              Provinsi        :   {{workAddr.province}}
                              <br/>
                              Kota            :   {{workAddr.city}}
                              <br/>
                              Kecamatan       :   {{workAddr.district}}
                              <br/>
                              Kelurahan       :   {{workAddr.subdistrict}}
                              <br/>
                              Desa            :   {{workAddr.village}}
                          </td>
                        </tr>
...
export default {
  ...
  methods : {
    refreshBorrowerDetails(){
        BorrowerDataService.viewIncomeSegments(this.uuid).then(res=> {
            this.incSeg = res.data;
            this.workAddr = res.data.workAddrRegionList;
            this.workPhone = res.data.workPhoneList;
        });
    },
  },
  created(){
