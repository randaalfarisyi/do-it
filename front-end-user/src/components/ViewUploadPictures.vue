...
<div>
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb mt-4 mx-4">
          ...
          <li class="breadcrumb-item active" aria-current="page">Upload Pictures</li>
        </ol>
    </nav>
          ...
          <div class="col-md-3">
              <h3 class="m-4">nama</h3>
                <ul class="list-group ml-3">
                  ...
                  <li class="list-group-item active side-tab"><a>Upload Picture</a></li>
                </ul>
              <button type="button" class="btn btn-outline-info ml-3 my-3 btn-block">Kembali</button>
              </div>
          <div class="col-md-9 mt-5">
              <h2 class="m-4">Uploaded Pictures</h2>
                <table class="table table-borderless">
                    <tbody>
                        <tr>
                            <th scope="row">KTP</th>
                            <td>{{upldPic.idCard}}</td>
                        </tr>
                        <tr>
                            <th scope="row">Foto Diri</th>
                            <td>{{upldPic.selfie}}</td>
                        </tr>
                        <tr>
                            <th scope="row">Foto Diri dengan KTP</th>
                            <td>{{upldPic.selfieIdCard}}</td>
                        </tr>
                        <tr>
                            <th scope="row">NPWP</th>
                            <td>{{upldPic.taxCard}}</td>
                        </tr>
                        <tr>
                            <th scope="row">Slip Gaji</th>
                            <td>{{upldPic.paySlip}}</td>
                        </tr>
                    </tbody>
                </table>
...
export default {
  ...
  methods : {
    refreshBorrowerDetails(){
        BorrowerDataService.viewUploadPicture(this.uuid).then(res=> {
            this.upldPic = res.data;
            console.log(this.upldPic);
        });
    },
  },
  created(){
